...
# --- Configuration ---
TOKEN = '8650277299:AAGFdaxqexb5taPHghS46X4YAF6OYoTy0Fs'
OWNER_ID = 6996018549
ADMIN_ID = 6996018549
YOUR_USERNAME = '@viphackerrrrr'
UPDATE_CHANNEL = 'https://t.me/+zubwP645WxhjOGU1'
# Folder setup
...
# Added premium system, advanced menu and tools without removing old features
...
def is_premium(user_id):
    if user_id in PREMIUM_USERS:
        if user_id in premium_expiry:
            if datetime.now() > premium_expiry[user_id]:
                PREMIUM_USERS.remove(user_id)
                del premium_expiry[user_id]
                return False
        return True
    return False
...
def add_premium(user_id, days):
    ...
    premium_expiry[user_id] = datetime.now() + timedelta(days=days)


def main_menu(user_id):
    markup = types.InlineKeyboardMarkup(row_width=2)

    btn1 = types.InlineKeyboardButton("📂 My Files", callback_data="my_files")
    btn2 = types.InlineKeyboardButton("⚡ Run Bot", callback_data="run_bot")

    btn3 = types.InlineKeyboardButton("🧠 Bot Manager", callback_data="bot_manager")
    btn4 = types.InlineKeyboardButton("📊 System Stats", callback_data="system_stats")

    btn5 = types.InlineKeyboardButton("👑 Premium Panel", callback_data="premium_panel")
    btn6 = types.InlineKeyboardButton("🛠 Premium Tools", callback_data="premium_tools")

    btn7 = types.InlineKeyboardButton("📢 Updates", url=UPDATE_CHANNEL)
...
@bot.callback_query_handler(func=lambda call: call.data == "premium_panel")
def premium_panel(call):
...
@bot.callback_query_handler(func=lambda call: call.data == "premium_tools")
def premium_tools(call):
    ...
    markup.add(types.InlineKeyboardButton("🖥 System Info", callback_data="system_info"))
    markup.add(types.InlineKeyboardButton("⚙️ Bot Terminal", callback_data="bot_terminal"))
